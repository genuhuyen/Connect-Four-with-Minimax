...
public class Board {
    ...
    public int utility(){
      if(getGameState() == GameState.MAX_WIN){
        return (int)((10000.0 * numRows * numCols)/ movesMadeSoFar);
      }
      else if(getGameState() == GameState.MIN_WIN){
        return (int)((-10000.0 * numRows * numCols)/ movesMadeSoFar);
      }
      else{
      return 0;
    ...
    public int eval(){
      /*here - I modified your functions to take a third variable where it can detect
       how many rows I specify (ex. 2 in a row, 3 in a row) */
       
      //award 10 pts for each 3 in a row
      int xToWin = 3;
      int pt = 0;
      for (int r = 0; r < numRows; r++) {
          for (int c = 0; c < numCols; c++) {
            if (board[r][c] == 0) {
              ...
              if ((c <= numCols - xToWin && allMatchInARowX(r, c, xToWin))
                      || (r <= numRows - xToWin && allMatchInAColumnX(r, c, xToWin))
                      || (r <= numRows - xToWin&& c <= numCols - xToWin && allMatchInANorthEastDiagonalX(r, c, xToWin))
                      || (r <= numRows - xToWin&& c - xToWin >= -1 && allMatchInANorthWestDiagonalX(r, c, xToWin))) {
                  if (getPlayerToMoveNext() == Player.MIN) {
                      pt+=10;
                  } else if (getPlayerToMoveNext() == Player.MAX) {
                      pt-=10;
                  }
              }
          }
      ...
      //award 3 pt for each 2 in a row
      xToWin = 2;
      for (int r = 0; r < numRows; r++) {
          for (int c = 0; c < numCols; c++) {
            if (board[r][c] == 0) {
    ...
    private boolean allMatchInARowX(int row, int startcol, int xToWin) {
        for (int x = 0; x < xToWin - 1; x++) {
            if (board[row][startcol + x] != board[row][startcol + x + 1])
                return false;
        }
        return true;
    ...
    private boolean allMatchInAColumnX(int startrow, int col, int xToWin) {
        for (int x = 0; x < xToWin - 1; x++) {
            if (board[startrow + x][col] != board[startrow + x + 1][col])
                return false;
        }
    ...
    private boolean allMatchInANorthEastDiagonalX(int startrow, int startcol, int xToWin) {
        for (int x = 0; x < xToWin - 1; x++) {
            if (board[startrow + x][startcol + x] != board[startrow + x + 1][startcol + x + 1])
                return false;
        }
    ...
    private boolean allMatchInANorthWestDiagonalX(int startrow, int startcol, int xToWin) {
        for (int x = 0; x < xToWin - 1; x++) {
            if (board[startrow + x][startcol - x] != board[startrow + x + 1][startcol - x - 1])
                return false;
        }
